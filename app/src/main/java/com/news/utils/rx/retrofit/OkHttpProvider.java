package com.news.utils.rx.retrofit;

import android.text.TextUtils;

import com.news.app.App;
import com.news.utils.common.LogUtil;
import com.news.utils.common.NetworkUtil;
...
public class OkHttpProvider {
    ...
    private static OkHttpClient getOkHttpClient(Interceptor cacheControl) {
        ...
        //设置缓存
        File httpCacheDirectory = new File(App.getInstance().getCacheDir(), "httpCache");
    ...
    private static class UserAgentInterceptor implements Interceptor {
        ...
        UserAgentInterceptor(String userAgentHeaderValue) {
            if (userAgentHeaderValue == null || userAgentHeaderValue.isEmpty()) {
                this.userAgentHeaderValue = "userAgentHeaderValue = null";
            } else {
    ...
    private static class LoggingInterceptor implements Interceptor {
        ...
        public Response intercept(Chain chain) throws IOException {
            ...
            LogUtil.e(this.getClass().getSimpleName(), String.format(Locale.CHINA, "Sending request %s on %s%n%s", request.url(), chain.connection(), request.headers()));

            Response response = chain.proceed(request);

            long t2 = System.nanoTime();
            LogUtil.e(this.getClass().getSimpleName(), String.format(Locale.CHINA, "Received response for %s in %.1fms%n%s", response.request().url(), (t2 - t1) / 1e6d, response.headers()));
